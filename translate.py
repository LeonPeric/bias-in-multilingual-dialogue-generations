...
def Translate(
    word, translator, target_lang, n_attempts=10, sleep_time=1
):  # Uses translators module to translate word, retrying a specified number of times in the case of a connection error
    for attempt in range(n_attempts):
        try:
            out = ts.translate_text(
                word, translator=translator, from_language="en", to_language=target_lang
            )
        except:
            print(
...
if False:
    ...
    for cat in nouns:
        for i in range(
            len(nouns[cat])
        ):  # List of lists containing singular and plural versions of nouns
...
if False:
    with open(
        os.path.join(path, version, "descriptors.json"), "r", encoding="utf-8"
    ) as file:
        descriptors = json.load(file)
        for cat in descriptors:
            for subcat in descriptors[cat]:
                for i in range(
                    len(descriptors[cat][subcat])
                ):  # List of words, but might also be dictionaries
                    word = descriptors[cat][subcat][i]
                    if isinstance(word, dict):
                        if list(word.keys())[0] == "descriptor":
                            descriptors[cat][subcat][i]["descriptor"] = Translate(
                                word["descriptor"], translator, target_lang
                            )
                    else:
                        descriptors[cat][subcat][i] = Translate(
                            word, translator, target_lang
                        )

    with open(
        os.path.join(target_path, "descriptors.json"), "w", encoding="utf-8"
    ) as file:
        json.dump(descriptors, file, indent=4)

if False:  # Gives error if two sentences are translated to the same
    with open(
        os.path.join(path, version, "sentence_templates.json"), "r", encoding="utf-8"
    ) as file:
    ...
    with open(
        os.path.join(target_path, "sentence_templates.json"), "w", encoding="utf-8"
    ) as file:
...
def Placeholder(
    input, direction
):  # Replaces program's tags with placeholders or reverts them
...
if True:
    with open(
        os.path.join(path, version, "standalone_noun_phrases.json"),
        "r",
        encoding="utf-8",
    ) as file:
        nps = json.load(file)
        for cat in nps:
            for i in range(
                len(nps[cat])
            ):  # List of nps, but might also be dictionaries
                np = nps[cat][i]
                if isinstance(np, dict):
                    sg = Placeholder(np["noun_phrase"], "placeholder")
                    nps[cat][i]["noun_phrase"] = Placeholder(
                        Translate(sg, translator, target_lang), "original"
                    )
                    if "plural_noun_phrase" in np:
                        pl = Placeholder(np["plural_noun_phrase"], "placeholder")
                        nps[cat][i]["plural_noun_phrase"] = Placeholder(
                            Translate(pl, translator, target_lang), "original"
                        )
                else:
                    nps[cat][i] = Placeholder(
                        Translate(
                            Placeholder(np, "placeholder"), translator, target_lang
                        ),
                        "original",
                    )

    with open(
        os.path.join(target_path, "stand_alone_noun_phrases.json"),
        "w",
        encoding="utf-8",
    ) as file:
